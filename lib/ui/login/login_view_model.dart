    ...
    try {
      ...
      try {
        ...
        if (await isKakaoTalkInstalled()) {
          ...
          authCode =
              await AuthCodeClient.instance.authorize(redirectUri: KakaoSdk.redirectUri);
          print('카카오계정 로그인 인가코드: $authCode');
        }

        // 인가코드를 받았다면 서버에 전송
        if (authCode != null) {
          final result = await _getAuthUseCase.call(code: authCode);
